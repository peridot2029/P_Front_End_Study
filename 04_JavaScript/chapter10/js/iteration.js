...
var beverage = document.querySelectorAll('.ediya-menu__item');
var i = 0;
...
var menu__items = document.querySelectorAll('.ediya-menu__item');

for (var i = 0; i < menu__items.length; i++) {
    var item = menu__items[i];
    // item은 문서객체 이기 때문에 color를 추가한다.
    // JavaScript는 객체 속성 값을 설정하기 쉽다
    item.color = colors[i];
...
var beverage__01 = {
    name: '벚꽃라떼',
    type: 'ICED',
    photo: 'file:///C:/Users/audwo/Desktop/S_Front_End_Study/04_JavaScript/chapter10/images/ICED-%EB%B2%9A%EA%BD%83%EB%9D%BC%EB%96%BC.png',
    width: '328px',
    height: '328px',
};
...
for(var star = 5; star >= 1; star--){
  ...
  for(var y =9; y >= 1; y-=2){
    console.log(' '.repeat((9-y)/2) + '*'.repeat(y));
  }