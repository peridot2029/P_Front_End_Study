...
function scopeFn() {
  var phone = 'iPhone'; // 함수 내에 있는 지역변수
  console.log('scopeFn phone: ',phone); 
}
scopeFn(); // 함수 실행이 종료되면 더 이상 지역변수는 사용할 수 없다.
console.log('phone :',phone);

// #2. 함수 내에서 var 키워드를 사용하지 않은 변수는 지역 변수로 처리지 않는다.
function myfun() {
   tablet = 'Note XII';  
   console.log(tablet);
}
...
myfun(); 
console.warn('올바르지 않은 예제 :', tablet);
...
// 네임스페이스를 활용한 변수 대용
song.phone = 'iPhone 8';
song.email = 'songsong@gmail.com';
// 네임스페이스를 활용한 함수 대용
song.getPhone = function () {
  return this.phone;
};
...
var radio = '보는 라디오';
console.log('radio: ', radio); // '보는 라디오'

if (true) {
  // var radio = '듣는 라디오'; // 블록 영역 형성 X
  let radio = '듣는 라디오';    // 블록 영역 형성 O
  console.log('radio let: ', radio); // '듣는 라디오'
}
